...
class Model3DWithLinear(torch.nn.Module):
    def __init__(self, base_model):
        super().__init__()
        self.base = base_model
        self.proj = torch.nn.Linear(64, 768).cuda()   # adjust if output dim differs

    def forward(self, x, mask=None):
        feats = self.base(x).feat[mask] if mask != None else self.base(x).feat      # output: B x N x 64 or B x 64
        # print(feats.shape)
        out = self.proj(feats)
        return out
...
def distill(config):
    ...
    print(model_3d)
    total = sum(p.numel() for p in model_3d.parameters())
    trainable = sum(p.numel() for p in model_3d.parameters() if p.requires_grad)
    ...
    for i in range(config.distill.epochs):
        for j, batch in enumerate(loader): 
            ...
            grid_coord = locs[:, 1:4].clone().int()    # (N, 3)
            grid_coord += (torch.rand(3) * 100).type_as(grid_coord)
...
def eval(config, model_3d, model_2d, voxelizer, iter):
    ...
    if config.model.dynamic:
        ...
        # )
        eval_config.model.model_dir = os.path.join(eval_config.model.model_dir, "scene0000_00")
    ...
    grid_coord = torch.from_numpy(locs).int().cuda()
    ...
    sinput = {
        ...
        "coord": coord.cuda(),
        "offset": torch.tensor([feat.shape[0]]).int().cuda(),
    }

    features_semantic = model_3d(sinput)
    features_semantic = features_semantic / features_semantic.norm(dim=-1, keepdim=True)
    # print(features_semantic.shape)
